...
fn instr_template_decl<'a>() -> impl Parser<'a, TokenStream<'a>, ImmNode> {
    attached_comment()
        ...
        .map(
            |(comments, (kw, aliens1, name, aliens2, params, parent, aliens3))| {
                (comments, kw, aliens1, name, aliens2, params, parent, aliens3)
            },
        )
        .and_then(struct_body())
        .map(
            |((comments, kw, aliens1, name, aliens2, params, parent, aliens3), body)| {
                (
                    comments, kw, aliens1, name, aliens2, params, parent, aliens3, body,
                )
            },
...
fn flag<'a>() -> impl Parser<'a, TokenStream<'a>, ImmNode> {
    attached_comment()
        .and_then(token(SyntaxKind::FlagKw))
        .and_then(eat_until(SyntaxKind::Identifier))
        .and_then(token(SyntaxKind::Identifier))
        .and_then(eat_until(SyntaxKind::Semicolon))
        .and_then(token(SyntaxKind::Semicolon))
        .flat()
        .map(|(comments, flag_kw, aliens1, name, aliens2, semicolon)| {
            let mut elements = comments;
...
fn enum_<'a>() -> impl Parser<'a, TokenStream<'a>, ImmNode> {
    attached_comment()
        ...
        .map(|(comments, (kw, aliens1, name, aliens2))| (comments, kw, aliens1, name, aliens2))
        .and_then(enum_variants())
        .map(|((comments, kw, aliens1, name, aliens2), variants)| {
